...
> **版本**：v4.1 — 二维码识别全链路优化 (4 方案同步: 多帧重试 + 传感器调优 + 图像预处理 + EC 升级)
> **日期**：2026-06-16
...
| `POST` | `/api/camera/scan` | 摄像头帧二维码扫码（P0 多帧重试：3 帧×80ms + 重复帧跳过，识别率大幅提升） |
...
每袋化肥包装上贴有二维码标签（version=4, EC=H 30% 纠错, 33×33 模块），内含 JSON 格式的物料身份信息：
...
| 3 | 将二维码对准 ESP32-CAM | pyzbar 实时解码，0.5 秒冷却防重复（支持高频扫码） |
...
- ✅ 二维码扫码入库/出库（化肥标签 FERT-20260611-001~006）+ CLAHE 增强 + 锐化核预处理 + 多帧重试
...
- ✅ **CameraWebServer/** ESP32-CAM Arduino 工程源码 (OV2640, contrast=2 锐化边缘 + aec2 快速曝光 + gainceiling=2X 降噪，针对 QR 扫码优化)
